...
DEFAULT_OUTPUT_ROOT = Path.home() / "Downloads" / "WeixinReplayMP3"
...
def output_path(short_id: str, explicit: str = "") -> Path:
    if explicit:
        return Path(explicit).expanduser().resolve()
    return (DEFAULT_OUTPUT_ROOT / f"weixin_{short_id}.mp3").resolve()


def run_dir(short_id: str, mode: str) -> Path:
    suffix = "" if mode == "auto" else f"-{mode}"
    return private_dir(
        ROOT / "work" / "sensitive-artifacts" / "weixin-link-runs" / f"{short_id}{suffix}"
    )
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    output = output_path(short_id, args.output)
    artifacts = run_dir(short_id, mode)
